    ...
    <div className="min-h-screen flex flex-col bg-slate-50">
      ...
      {currentStage === Stage.FINAL_PREVIEW && activeGroup && (
        <div className="preview-overlay">
          ...
          {/* Floating Actions (Strictly No-Print) */}
...
const AssessmentPage: React.FC<{ title: string, group: GroupData, curriculum: CurriculumConfig, observations: Record<number, string>, settings: PreviewSettings }> = ({ title, group, curriculum, observations, settings }) => (
  ...
  >
    ...
    <div className="mt-2 border-t-[2px] border-black pt-1.5 flex justify-between items-center text-[11px] font-black px-2">
      ...
      <div className="text-[12px] font-black">إمضاء الأستاذ: ____________________</div>
    </div>
  </div>
);
...
const PerformanceCardPage: React.FC<{ group: GroupData, curriculum: CurriculumConfig, observations: Record<number, string>, settings: PreviewSettings }> = ({ group, curriculum, observations, settings }) => (
  ...
  >
    ...
    <table className="flex-grow mb-2 w-full border-collapse">
      <thead>
        ...
        <tr className="h-6 bg-slate-100 text-[10px] font-black">
          ...
          <th className="w-14 p-0 border-black border-[2px]">الحضور</th>
          <th className="w-14 p-0 border-black border-[2px]">اللباس</th>
          <th className="w-14 p-0 border-black border-[2px]">السلوك</th>
          <th className="w-14 p-0 border-black border-[2px]">المشاركة</th>
          <th className="w-14 p-0 border-black border-[2px]">التنسيق</th>
          <th colSpan={2} className="p-0 border-black border-[2px]"></th>
        </tr>
      </thead>
      <tbody>
    ...
    <div className="mt-2 flex justify-end pl-12 border-t-[2px] border-black pt-2">
  ...
  return (
    <div 
      ...
      style={{ 
        ...
        paddingLeft: `${settings.marginLeft}mm`, 
        paddingRight: `${settings.marginRight}mm`,
        width: '297mm', // Fixed A4 landscape width as per code snippet
        height: '210mm'
      }}
    >
      <div 
      ...
      <div className="mt-2 flex justify-between items-center text-[11px] font-black p-3 bg-slate-900 text-white rounded-xl border border-slate-800 shadow-xl border-t-[3px] border-black">
        <div className="flex gap-8">
          <span className="flex items-center gap-1">غ: غائب</span>
          <span className="flex items-center gap-1">م: متأخر</span>
          <span className="flex items-center gap-1">ب: بدون لباس</span>
          <span className="flex items-center gap-1">ض: مريض</span>
          <span className="flex items-center gap-1">X: معفي</span>
        </div>
